/**
 * Client auth gate of the /v1beta group (S2d2 section 2.2).
 *
 ...
 * `Invalid API key` - both in the string-`error` shape, not the OpenAI
 * object shape. With no keys configured the group is open.
 */
 ...
 * all fails missing. An empty key set leaves the group open (matches the
 * reference's no-keys-configuration behavior).
 */
export function authenticateV1Beta(
  headers: HeaderList,
  query: URLSearchParams,
