/**
 * Upstream header assembly for the OpenAI-compatibility executor (the
 * recorded chat wire).
 *
 ...
 * transport owns `Accept-Encoding: gzip`. Emission order is pinned by
 * the recorded wire log: `Host`, `User-Agent`, `Content-Length` first,
 * the remaining names in ASCII order, `Accept-Encoding` last.
 */
import type { HeaderList } from './types'

/** User-Agent of the openai-compatibility executor (recorded). */
...
export interface UpstreamHeadersInput {
  ...
  /** Stream mode adds `Accept` and `Cache-Control` (recorded pair). */
 ...
 * recorded non-stream tail `Authorization, Content-Type`; the stream
 * tail `Accept, Authorization, Cache-Control, Content-Type`), then
