/**
 * JSON helpers for byte-exact wire output in the cla2oai direction.
 *
 * Recorded behaviors this module encodes:
 *
 * - every string the GATEWAY writes into the upstream body or a
 *   downstream event is JSON-encoded with HTML escaping (`<` ->
 *   `\u003c`, `>` -> `\u003e`, `&` -> `\u0026`; recorded: the
 *   `<system-reminder>` wrapper text arrives upstream escaped);
 * - client-owned tool arguments travel as RAW bytes - original spacing
 *   and key order survive, so the serializer splices unvalidated JSON
 *   text verbatim;
 * - values the reference re-serializes through a Go map (tool schemas,
 *   parsed tool arguments) marshal with alphabetically sorted keys, so a
 *   deep key sorter reproduces that shape;
 ...
 * arguments are embedded with the client's original formatting, so they
 * are never re-parsed on the way out.
 */
 ...
 * escaping: `<`, `>` and `&` never appear literally inside
 * gateway-written strings.
 */
 ...
 * the shape a Go map marshal produces for values the reference parses
 * and re-embeds (tool schemas, tool-call arguments).
 */
