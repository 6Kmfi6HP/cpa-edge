...
| S2d7 | 32 (23 + 2 valid-args + 6 gate-fix goldens + 1 countTokens-cooldown ruling) | `tests/fixtures/S2d7/` | mock_claude.py: control `variant` tool/errstream/maxtokens/tool-valid/empty/nostart/truncated/malformed (+ mocklib RAW pre-framed SSE line) | verbatim model name does NOT resolve on /v1beta; cooldown surfaces 429 (not 500); invalid input_json_delta spliced RAW (invalid-JSON frame, top-level finishReason, duplicate usageMetadata) — valid args confirm clean-shape hypothesis; countTokens + 4 stream validators match expected strings exactly; empty upstream + STREAM client => 500 empty_stream pre-commit (NOT 200/SSE); ruling S2d7-31: ACTIVE cooldown GATES countTokens (429 model_cooldown, 0 upstream calls) |
| S2d2 | 26 (20 + 6 round-2 gate fixes + basic-params re-record) | `tests/fixtures/S2d2/` | mock_openai.py: control `scenario` (8 byte-exact scripts) + mocklib raw-string error_body | thinkingLevel auto -> reasoning_effort medium (no passthrough); cooldown 429 + reset 4s + Retry-After 4; models/-prefixed single-model GET 404s; alt=json -> Content-Type text/plain; clamps low/low/high/high/low/low/high confirmed; 400 validator bodies byte-exact; multi-tool single finish frame (parts order nondeterministic) |
| S2d5 | 26 (24 + 2 round-1 gate goldens) | `tests/fixtures/S2d5/` | responses_common.py: control `script` (10 scripts via s2d5_scripts.json) + mocklib x-mock-script | cooldown pair: R2 429 (not 500), reset 4/4s, last_upstream_error VERBATIM body (codex keeps raw, unlike openai-compat summary); rate-limit window outlasts the reported 1s — leave >=5s between error cases; gate goldens CONFIRMED: 8-branch oneOf->enum rewrite + function-wins type selection; empty-incomplete -> pre-commit 502 (0 tokens) |
