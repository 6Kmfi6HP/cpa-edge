/**
 * Declarative route table and matcher (S1 section 3) with the one
 * Cloudflare addition: `GET /v1/ws` (S7 F4 - the inbound WebSocket
 * route, always registered, GET only).
 *
...
export type RouteGroup =
  ...
  | 'client-ws' // /v1/ws: client auth gated by the ws-auth toggle
...
/**
 * The full route inventory (S1 sections 3.1-3.9). The order does not
 * matter for matching (patterns are mutually exclusive) except `:param`
 * routes after exact ones with the same prefix - the matcher checks
 * exact segments first.
 */
export const ROUTES: readonly RouteEntry[] = [
  ...
  // inbound WebSocket (S7 F4; S7-01/02/03 goldens)
  { method: 'GET', pattern: '/v1/ws', group: 'client-ws', id: 'ws-relay' },
...
/**
 * Trailing-slash redirect evaluation (S1 section 5). Fires only when the
...
/** Builds the pre-middleware redirect response (no CORS, S1 section 5). */
