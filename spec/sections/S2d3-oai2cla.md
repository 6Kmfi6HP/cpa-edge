...
Key order with optional fields present: `model`, `max_tokens`, `messages`, `metadata`, `thinking`, `top_p`(deleted later — see §2.5), `stop_sequences`, `stream`, `system`, `tools`, `tool_choice`. **Recorded exception (case 20 step 2): in the adaptive-thinking case the `output_config` key lands AFTER `stream`** — the executor's thinking applier re-creates it, and re-created keys append at the end — recorded order `model, max_tokens, messages, metadata, thinking, stream, output_config`. Byte-exact emulation follows the recording, not the translator's internal set order. Recorded: t1/t2 bodies have exactly `model,max_tokens,messages,metadata,stream`. Byte-exactness (minus dynamic fields) is contract material.
...
| `response_format` | `type:"json_object"` or `json_schema` → an extra system text block is APPENDED at the end of the system blocks with a fixed instruction string. **`json_object` — the normative sentence (recorded, case 27, byte-exact):** `You must format your entire response as a valid JSON object. Do not include any explanations, markdown code blocks (such as ```json), or any text outside of the JSON object.` (identical to the json_schema-without-schema fallback). `json_schema`: `You must format your entire response as valid JSON that conforms strictly to the following JSON schema:` + optional `Schema Name: <n>` / `Schema Description: <d>` lines + `JSON Schema:` + the raw `schema` JSON + the same closing sentence as json_object. The appended block is the LAST system block, so the default cache-control marker lands on it. Other/absent → nothing. Evidence: `internal/translator/common/claude_system.go` `BuildClaudeStructuredOutputInstruction`; fixture `tests/fixtures/S2d3/s2d3-respformat-json-object/` (case 27). |
...
| `metadata.user_id` derivation | Priority: caller `metadata.user_id` → caller `user` → hash. Hash seed: `prompt_cache_key:<v>` → else `session_id|sessionId` → else conversation ids → else `content:<first user-message text (parts joined with \n)>` → else `model:<model>[;instructions:...][;system variants]` → else literal `"unknown"` **Chain reduction for this surface**: a Chat Completions body can only ever exercise `metadata.user_id` → `user` → `prompt_cache_key` → first-user-message content → `model`(+`system`); the generic helper's `session_id`/`conversation-id`/`input`/`contents` branches are unreachable from a Chat request (they exist for Responses/Gemini/Claude-native surfaces). | Seed → `sha256` hex (64 chars). Evidence: `internal/translator/common/claude_user_id.go` `DeriveClaudeUserID`. Recorded: t1 `metadata.user_id = 120226d8c5cb... = sha256("content:Say hello")` — deterministic and recomputable in tests. |
...
| `Retry-After` | **NOT forwarded on the direct upstream-status response** (recorded absent, case 26 step 1): the classified `statusErr`/`claudeRateLimitError` carries no response headers, and `coreauth.SafeResponseHeaders` materializes `Retry-After` only for cooldown/home errors. The cooldown FOLLOW-UP response does carry `Retry-After` (§5.4). Evidence: `sdk/cliproxy/auth/home_concurrency.go` `SafeResponseHeaders`. |
...
### 7.2 Golden-sample index (27 cases — all recorded)

(All 27 rows are RECORDED fixtures under `tests/fixtures/S2d3/<case-id>/`; rows 19–26 are the round-1 follow-up batch; row 27 is the post-admission json_object sentence pin.)
...
| 27 | `s2d3-respformat-json-object` | RECORDED (post-admission authorization): exact `json_object` instruction sentence as the appended system block + default cache marker | happy text |
