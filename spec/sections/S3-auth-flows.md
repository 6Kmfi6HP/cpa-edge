...
Section status: REVISION 3 after gate rounds 1–2 (review: `reports/adversary/S3.md`; round 1: B1–B6 + N1–N10 applied with goldens; round 2: B1–B6 verified, final amendments F1–F3 applied — vertex auth-file existence, ws-auth scope, header matrix scope). Anchor: CLIProxyAPI v7.3.4, commit `8335eac731946bd4eff18f500653f93736df53d6`.
...
Separately, config `ws-auth` (default true) gates ONLY the generic websocket-attachment routes (the wsrelay route `/v1/ws` and plugin-attached WS upgrades, served with a conditional auth wrapper) — NOT the `/v1/realtime*` surface, which always uses the middlewares above (config schema in S6, management route in S5).
...
1. Response headers set on every management response (before any check, including 401/403 bodies; unconfigured 404s excepted — see §5): `X-CPA-VERSION`, `X-CPA-COMMIT`, `X-CPA-BUILD-DATE`, `X-CPA-SUPPORT-PLUGIN` — emitted with Go-canonical capitalization `X-Cpa-Version: v7.3.4`, `X-Cpa-Commit: 8335eac`, `X-Cpa-Build-Date: 2026-09-15T14:07:06Z`, `X-Cpa-Support-Plugin: 1` (recorded). These headers do NOT appear on `/v1`/`/v1beta` auth rejections (those carry only the global CORS block from S1). `X-Cpa-Trace-Id` is never emitted on auth or management surfaces (it is only listed in `Access-Control-Expose-Headers`). Values are build-info constants; masked as dynamic in goldens.
2. `clientIP = c.ClientIP()`. PINNED CONTRACT (B6/N8): the server never calls gin's `SetTrustedProxies`, so gin v1.10.1 defaults apply — `New()` sets trusted proxies `0.0.0.0/0` + `::/0` (trust all) with `ForwardedByClientIP` on and remote-IP headers `X-Forwarded-For` then `X-Real-IP`. Exact resolution (gin `Engine.validateHeader`): the `X-Forwarded-For` list is walked RIGHT to LEFT and the walk returns the first entry that is leftmost (`i == 0`) or not a trusted proxy — under trust-all this is the LEFTMOST parseable entry; an unparseable entry aborts the walk and resolution falls to `X-Real-IP`, then the TCP remote address. `localClient` iff that IP is `127.0.0.1` or `::1`. Consequence: any client (including a remote one) can present `X-Forwarded-For: 127.0.0.1` and be treated as local, and the ban counter is keyed by the resolved (spoofable) IP. This is recorded reality, not an endorsement. Replay topology: the `s3-mgmt-remote-disabled` golden pins both branches deterministically with explicit `X-Forwarded-For` headers (`10.0.0.99` -> 403 remote-disabled; `127.0.0.1` -> 200) and is byte-replayable from ANY network position.
...
OAuth callback handshake files: `.oauth-<provider>-<state>.oauth`, JSON `{code, state, error}`, atomic publish, removed after consumption; `.oauth-*` naming keeps them out of the `*.json` walk. The `gemini` skip is case-insensitive (`type: "Gemini"` is skipped too). Vertex auth files exist (type:`vertex`: service_account, project_id, email, location; file name `vertex-<sanitized project>.json`) but are created only by the management import endpoint — schema owned by S6, HTTP surface by S5; no login flow in S3.
...
**Round-1 delivery (oracle-runner-2, recorded 2026-09-15 against the anchored image on an isolated stack; instance port and `Host`/`redirect_uri` port occurrences are masked dynamic fields):** the initial 38 recordable cases under `tests/fixtures/S3/<case-id>/` (57 response transcripts, 47 request files at that point; `s3-mgmt-remote-disabled` was superseded by the round-2 re-record below). Layout per RECIPES with the multi-step extension: single-request cases = `meta.yaml` + `request.http` + `downstream.md`; multi-request cases = `request.http`/`request-N.http` and `downstream.md`/`downstream-<step>.md`; repeated steps = `downstream-<step>r<i>.md`. `meta.yaml` carries the full request/response file map with `(step, repeat, status)` per response, `$S` substitution values, the instance config fragment, and `dynamic_fields`.
...
- realtime shapes (3): `s3-realtime-{unauth,invalid-key,ek-invalid-secret}`.
- management authz (8) + device-auth egress-blocked (3): `s3-mgmt-{missing-key,invalid-key,valid-x-management-key,valid-bearer,remote-disabled,unconfigured-404,ban-reset-on-success,ban-after-5-invalid}` and `s3-device-auth-url-egress-blocked-{kimi,xai,meta}`.
- plain callback routes (4): `s3-oauth-callback-{anthropic,codex}-unknown-state`, `s3-oauth-callback-devin-{missing,unknown}`.
- management oauth-callback ladder (7): `s3-mgmt-oauth-callback-{invalid-body,missing-state,invalid-state,missing-code,unknown-state,invalid-redirect-url,persist-fail}`.
...
**Round 2 DELIVERED (oracle-runner-4, recorded 2026-09-15; review `reports/adversary/S3.md`):** all 7 recordings landed byte-exact — `s3-realtime-ek-invalid-secret` (B1: `ek_` garbage → 401 `invalid_realtime_client_secret`; confirms client-route 401s carry NO `X-Cpa-*` headers), `s3-mgmt-oauth-callback-invalid-redirect-url` (B2: 400 `invalid redirect_url`), `s3-mgmt-oauth-callback-persist-fail` (B2: 2 steps; 500 `failed to persist oauth callback` with a read-only auth dir; also proves a clean boot with the auth dir mounted `:ro`), `s3-device-auth-url-egress-blocked-{kimi,xai,meta}` (B5: deterministic fast 500s — `failed to generate authorization url` / `failed to start device authorization flow` ×2 — via `--add-host <vendor>:127.0.0.1` conn-refused), and `s3-mgmt-remote-disabled` RE-RECORDED with explicit `X-Forwarded-For` headers (B6: `10.0.0.99` → 403 remote-disabled; `127.0.0.1` → 200 `{"api-keys":[...]}`), replacing the round-1 fixture and pinning the trust-all-proxies XFF contract. Totals now: **44 case directories, 65 response transcripts, 55 request files**. Raw evidence: `_cpa_edge_ref/run4/probes/S3-round2/`; oracle notes: `reports/oracle/worker4.md`.
