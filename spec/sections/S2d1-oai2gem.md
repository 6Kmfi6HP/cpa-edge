...
**Dropped fields (intentional non-equivalence):** `stop`, `tool_choice`, `parallel_tool_calls`, `seed`, `frequency_penalty`, `presence_penalty`, `logprobs`, `top_logprobs`, `user`, `stream_options`, `store`, `metadata`, `service_tier`, the LEGACY function fields `functions` and `function_call` (this direction has NO handling for them — only `tools`/`tool_calls` are translated), and every other unmapped field. `tool_choice` in particular is NOT translated to `toolConfig` in this direction (it IS in claude→gemini — intentional asymmetry).
...
- **Synthetic tool-result turn:** immediately after an assistant content that had tool_calls, ONE user content `{"role":"user","parts":[…]}` is emitted with one `functionResponse` part **per assistant tool_call that carries a non-empty `id` AND a non-empty `function.name`** (and `type:"function"`): `{"functionResponse":{"name":<sanitized function name>,"response":{"result":<content>}}}`. A `tool` message whose `tool_call_id` matches that id fills `result`: a string content becomes a JSON string (its raw bytes verbatim); a JSON-object content is embedded raw as an object. When NO matching tool message exists the part is STILL emitted with `result` as an empty JSON object `{}` (the gateway's `"{}"` default is embedded raw via sjson). `tool` messages themselves are never emitted as standalone contents; a `tool` message with no matching tool_call id is dropped. Assistant tool_calls without an `id` emit their functionCall part but no functionResponse part.
...
- Multi-candidate upstream chunks fan out to one downstream chunk per candidate, in candidate order, each frame's `choices[0].index` mirroring the candidate `index` (recorded, C22). The `usage` object is built once per upstream chunk and therefore appears IDENTICALLY on EVERY candidate's final frame when a terminal chunk carries usage (recorded, C22: both candidate-0 and candidate-1 final frames carry the same usage). Per-candidate state (`role`, tool-call index, finishReason, sawToolCall) is tracked independently by candidate index.
...
**Ruling R-SSE binding (SPEC §5):** the reference gateway re-chunks SSE transport frames nondeterministically between runs (decoded bodies identical). Every requirement in this section is a DECODED-EVENT-SEQUENCE requirement: contract tests compare the decoded SSE frames — `data:` payload bytes, in order, plus the terminal `[DONE]` — never raw TCP/curl chunk boundaries. The fixtures record the raw chunked stream for reference; the contract binds the decoded frames. The volatility whitelist (§6 dynamic fields) still applies on top.

Downstream frame grammar (byte-exact per decoded frame; volatile fields whitelisted in fixture `meta.yaml`):
1. First translated chunk commits `200` + SSE headers (§2.1). OPTIONAL (not golden-recorded): if the stream produces NO data before closing, the response is headers + `data: [DONE]\n\n` only (`openai_handlers.go handleStreamingResponse`).
...
- Note: `BuildErrorResponseBodyWithError`'s terminal-auth branch (`upstream_authentication_required`) is UNREACHABLE on this pipeline — terminal auth errors require OAuth refresh paths that the `gemini-api-key` executor never produces (it exists for OAuth-backed upstreams).
- Request-fault classification: the full matrix (which upstream failures rotate/cool credentials vs. fail-fast) is **deferred wholesale to S4** (source: `internal/clienterror/client_error.go IsRequestFault` — 400/409/413/422 bodies are request faults that fail-fast without rotation or cooldown; 402 and 429 are authoritative non-request-faults even over generic bodies; 401 carrying an `authentication_error` body and any `model_not_found`-code body are also non-request-faults). Observable outcomes pinned on THIS pipeline: upstream 429 → credential cooldown (C15→C16); upstream 400 INVALID_ARGUMENT → fail-fast, no cooldown (C17 recorded cold in its own window).
...
| `C22-stream-n2-candidates/` | multi-candidate STREAMING (gate B1): n=2 → candidateCount 2; per-candidate frame fan-out in order; both final frames carry finish_reason 'stop' + the SAME duplicated usage | recorded |
| `C23-stream-c1-only-finish/` | terminal chunk with ONLY candidate 1 finished: candidates.0-keyed usage filter hides usage → NO frame ever carries finish_reason or usage | recorded |
