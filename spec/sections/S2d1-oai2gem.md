...
| Stream success | First translated chunk commits `200` with `Content-Type: text/event-stream`, `Cache-Control: no-cache`, `Connection: keep-alive`, `Access-Control-Allow-Origin: *`; then one `data: <chunk>\n\n` frame per translated chunk, flushed individually | `openai_handlers.go handleStreamingResponse setSSEHeaders`, `sdk/api/handlers/stream_forwarder.go` |
...
2. A thinking suffix `alias(<suffix>)` is legal: suffix is stripped for provider lookup, and the configured upstream model keeps the suffix appended. `internal/util/…` `sdk/api/handlers/handlers_routing.go getRequestDetailsWithOptions`, `sdk/cliproxy/auth/conductor_models.go preserveRequestedModelSuffix`.
...
| `reasoning_effort` (string) | `generationConfig.thinkingConfig` | `"auto"` → `thinkingBudget: -1`; any other non-empty value → `thinkingLevel: <lowercased/trimmed>`. Then model-aware pass (§3.2 R4): for registry-unknown (user-defined) models the level is converted to a **budget** and written as `thinkingConfig.thinkingBudget` only: none→0, auto→-1, minimal→512, low→1024, medium→8192, high→24576, xhigh→32768, max→128000; `thinkingLevel` is deleted and `includeThoughts` is never added. Evidence: `gemini_openai_request.go`, `internal/runtime/executor/helps/model_capabilities.go ApplyRequestThinking`, `internal/thinking/apply.go applyUserDefinedModel/normalizeUserDefinedConfig`, `internal/thinking/convert.go levelToBudgetMap`, `internal/thinking/provider/gemini/apply.go applyBudgetFormat` |
...
**Cooldown interaction (cross-listed S4; client-visible shape pinned here):** an upstream 429 puts the credential into a short (~1s) rate-limit cooldown that `transient-error-cooldown-seconds: -1` does NOT disable (ORACLE-WIRE). The first request still returns the upstream 429 verbatim; a request hitting the cooldown gets `500` `{"error":{"code":"model_cooldown","last_upstream_error":"<verbatim upstream body>","message":"All credentials for model <m> are cooling down via provider <p> (last error: …)"}}` (shape per ORACLE-WIRE; exact bytes pinned by fixture S2d1-C13).

## 6. Golden samples index

All fixtures recorded by @oracle-runner against the pinned image, following the RECIPES layout (`meta.yaml`, `request.http`, `downstream.md`, `upstream.jsonl`, `mock-response.json`). Target dir: `tests/fixtures/S2d1/<case-id>/`. Case definitions: `spec/recordings/S2d1.cases.json`.

| Fixture (tests/fixtures/S2d1/…) | Pins | Status |
|---|---|---|
| `C01-nostream-basic/` | systemInstruction extraction; sampling → generationConfig; safetySettings injection; upstream URL/headers; `chat.completion` envelope incl. `id:""`, `created:0`, model echo = upstream name | pending |
| `C02-nostream-multiturn/` | multi-turn contents roles; trailing system → user content; last-turn-user (no synthetic turn) | pending |
| `C03-nostream-tools-history/` | functionDeclarations rename+clean; functionCall parts + literal thought signature; synthetic functionResponse user turn; tool message consumption | pending |
| `C04-nostream-tool-call/` | response functionCall → `message.tool_calls` (no index), finish_reason/tool_calls override, native_finish_reason tool_calls | pending |
| `C05-nostream-max-tokens/` | max_tokens → maxOutputTokens; MAX_TOKENS → finish_reason `max_tokens` | pending |
| `C06-nostream-usage-details/` | thought parts → reasoning_content; thoughtsTokenCount/cachedContentTokenCount → usage details | pending |
| `C07-nostream-reasoning-effort/` | reasoning_effort low → thinkingBudget 1024 (user-defined model path) | pending |
| `C08-nostream-model-suffix/` | alias thinking suffix → suffix-stripped upstream model + thinkingBudget 24576 | pending |
| `C09-nostream-n2-candidates/` | n>1 → candidateCount; 2 candidates → 2 choices | pending |
| `C10-nostream-image-data-url/` | image_url data: → inlineData + literal thoughtSignature on user image part | pending |
| `C11-stream-basic/` | 3-chunk SSE sequence: role+content chunks (model `"model"` mid-stream), final chunk = finish_reason+usage+modelVersion merge, `data: [DONE]` | pending |
| `C12-stream-tool-call/` | stream tool_calls chunks (index, dynamic id), finish_reason `tool_calls` + native_finish_reason `stop` asymmetry | pending |
| `C13-stream-max-tokens/` | stream MAX_TOKENS final chunk | pending |
| `C14-stream-no-terminal-merge/` | split finish/usage upstream chunks → no finish_reason, no usage, [DONE] only | pending |
| `C15-error-429-nostream/` | 429 + Gemini error JSON verbatim (status+body) | pending |
| `C16-error-429-cooldown/` | second request in cooldown → 500 model_cooldown shape (timing-sensitive) | pending |
| `C17-error-400-nostream/` | 400 INVALID_ARGUMENT verbatim pass-through | pending |
| `C18-stream-error-before-first-byte/` | stream upstream error → plain JSON 429 error response, NOT SSE | pending |
| `C19-stream-disconnect-mid-stream/` | 200 SSE kept; 2 chunks; terminal `data: {"error":…}` frame; NO [DONE] | pending |
| `C20-nostream-force-mapping/` | force-mapping: true → response `model` rewritten to client alias | pending |
...
3. **Catalog-dependent behavior** (startup model catalog from `raw.githubusercontent.com/router-for-me/models`): `capGeminiMaxOutputTokens` and registry-known thinking behavior vary with network availability. Goldens deliberately use non-catalog upstream model names (`gemini-mock-model*`) for determinism; catalog-known model behavior is specified but NOT golden-covered. A later mission may record it with a pinned local catalog.
