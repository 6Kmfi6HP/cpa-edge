...
- Shard path (built-in strategy active): `providerPrefersWebsocketTransport` covers **codex AND xai**; the preference applies when the downstream request is a WebSocket and no credential is pinned (`pinned_auth_id` empty). The websocket-enabled sub-view is searched **across ALL priority tiers first** — a ws-enabled credential in a LOWER priority tier beats a non-ws credential in the highest tier; within the chosen view the strategy order applies; an empty ws view falls back to the all-credentials view. `scheduler.go` (`pickSingleWithStrategy`, `highestReadyPriorityLocked`, `pickReadyAtPriorityLocked`, `buildReadyBucket`). Recorded: S4-21 (ws-enabled priority −5 credential attempted first on the downstream-WS leg; priority respected on the HTTP leg).
- The preference changes the PICK only; subsequent failure handling follows the normal classification. Recorded: S4-21 — the ws attempt's 404 body carried a request-fault type tag, so rotation STOPPED at the ws-enabled credential and the downstream WS client received one error frame relaying the 404 verbatim (no failover).
- Downstream-WS surface: the Responses websocket transport (`GET /v1/responses` upgrade; frames `{"type":"response.create","model",…,"input":[array],"stream"}` — string `input` rejected 400 `websocket request requires array field: input`; frames without `type` rejected 400 `unsupported websocket request type: `). `/v1/ws` is the wsrelay PROVIDER BRIDGE (registers an upstream websocket provider), not a request surface, and is out of S4 scope (S7). Recorded: S4-21.
...
| S4-21 | `…/S4-21-ws-transport-preference/` | downstream-WS (Responses ws transport) prefers the ws-enabled LOWER-priority codex credential across tiers; HTTP leg respects priority; the ws attempt's request-fault 404 stops rotation with an in-WS error frame relay | recorded |
| S4-22 | `…/S4-22-count-tokens-404-neutral/` | count_tokens 404 is availability-NEUTRAL (both requests sweep the pool, no cooldown) while the same 404 on chat cools 12 h (next chat → 503) | recorded |
| S4-23 | `…/S4-23-compact-fault-neutral/` | `/responses/compact` 5xx is NEUTRAL (both compact requests sweep all 3 credentials) while the same 500 on normal `/responses` cools (next → 503) | recorded |

23 recordable fixtures recorded (S4-01…S4-23). FIXTURE-DEFERRED (CREDENTIALED-ONLY, documented in §2.4/§2.5 with source evidence, no local golden possible): S4-D1 OAuth refresh-on-unauthorized; S4-D2 OAuth token-expiry demotion; S4-D3 Antigravity credits fallback.

Recorded notes that refine the rules above:
- S4-21: the downstream-WS surface is the Responses websocket transport (`GET /v1/responses` upgrade; frame `input` must be an array; a frame without `type` is rejected) — `/v1/ws` is the wsrelay provider bridge, not a request surface. The ws-preference pick is mandatory-observable: the ws-enabled LOWER-priority credential was attempted first; the subsequent 404 body (request-fault type tag) stopped rotation and was relayed as a single in-WS error frame (§5 rule applied to the WS transport too).
