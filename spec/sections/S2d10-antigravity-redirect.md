...
5. `request.sessionId`: injected only in the same branch as the `agent-` requestId (non-image, non-web_search): an existing `request.sessionId` wins; else the context-derived id; else a stable id hashed from the first user text; else random. Every form is a negative decimal string `-<int64>` (first 8 bytes of SHA-256, masked to 63 bits, base-10). Image and `web_search` requests carry no executor-injected `sessionId`. RECORDED confirmation: two different recordings (claude-family and gemini-family passes, different container boots, same first user text) produced the IDENTICAL `sessionId` `-6930635648078873014` — the stable-hash derivation is deterministic per first-user-text; keep masking it as a dynamic field anyway (it changes with content).
...
| S2d10-forwarder-port-busy | 51121 pre-occupied in-container (perl `IO::Socket::INET` listener via `docker exec`, technique recorded in meta `pre_occupation`) → `is_webui=1` auth-url returns 500 `{"error":"failed to start callback server"}` | `tests/fixtures/S2d10/S2d10-forwarder-port-busy/` | Date |
  ...
  Round-2 extension (GET get-auth-status after the 500 → `{"status":"wait"}`): **UNRECORDABLE — state not exposed.** The failed session's `state` is emitted on NO observable surface: the 500 body carries no 32-hex token (negative regex scan), and the container logs on the bind-failure path print only `Initializing Antigravity authentication...`, the forwarder bind error (`listen tcp 0.0.0.0:51121: bind: address already in use`), and the gin 500 access line. Evidence block: fixture meta `round2_extension_probe`. The B1 wait-forever behavior therefore stands as source-specified (§2.3), with negative-evidence backing only.
...
| S2d10-executor-request-shape-gemini | the OTHER family branch: gemini-family model, non-stream client | `tests/fixtures/S2d10/S2d10-executor-request-shape-gemini/` | upstream `POST /v1internal:generateContent` with NO query string (family split confirmed); envelope `{project, request:{contents, generationConfig:{}, sessionId}, model, userAgent:"antigravity", requestType:"agent", requestId}` — `generationConfig` present but EMPTY (`max_tokens:128` deletion rule visible), NO `request.toolConfig` (no VALIDATED injection); `:fetchAvailableModels` probe fires here too; chosen model `gemini-3.6-flash-high`; downstream 200 chat.completion |
...
- RECORDED (synthetic credential): `S2d10-executor-request-shape-gemini` closes the model-family matrix — the gemini-family non-stream branch takes `/v1internal:generateContent` (no query) while the claude-family always streams via `:streamGenerateContent?alt=sse`; both envelopes goldened.
...
9. RESOLVED: the gemini-family non-stream golden `S2d10-executor-request-shape-gemini` is recorded (2026-09-15): `/v1internal:generateContent` with no query string, empty `generationConfig` after `maxOutputTokens` deletion, no `toolConfig` injection. Both model-family branches now have executor-request goldens.
