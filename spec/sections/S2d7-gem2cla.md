...
- **Cooldown gating (recorded ruling, golden S2d7-31):** an ACTIVE credential cooldown gates `:countTokens` too — the per-credential cooldown check runs BEFORE the local token-count path. A countTokens request fired inside the cooldown window returns the §5.1 cooldown response (HTTP 429, `model_cooldown` body, `Retry-After: 1` — byte-identical to the S2d7-19 shape) and NO count is computed; the upstream is never contacted (wire delta 0). Evidence: `sdk/cliproxy/auth/conductor_execution.go` `ExecuteCount` selects credentials through the same cooldown-filtering path as Execute/ExecuteStream.
...
| S2d7-31-counttokens-cooldown | ACTIVE cooldown GATES :countTokens: 429 model_cooldown + Retry-After: 1 (S2d7-19 shape), no count, no upstream request (two-step fixture: cooldown setup + count probe) | no | error 429 setup + count probe |
