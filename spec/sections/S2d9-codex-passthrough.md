...
- Credential side effects (cooldown policy) are S4 scope, but two RECORDED FACTS are observable through this route and pinned by goldens:
  ...
  - UN-PINNED OBSERVATION (wire-note input to S4, not fixture-backed): an upstream 404 `model_not_found` appeared to ALSO put the credential into a cooldown that survives `transient-error-cooldown-seconds: -1` and outlasts multi-second waits — observed in the superseded worker-1 run (subsequent requests returned 503 cooldown-family errors with zero upstream calls until restart; that run's fixtures were wiped). No surviving S2d9 fixture pins the 404->cooldown transition; treat as un-recorded until S4 or a confirmation recording pins it (a cheap confirmation batch has been offered to the oracle).
...
RECORDED COMPLETE (2026-09-15/16, oracle worker-2, single provenance; worker-1's superseded cancelled run wiped): all 17 cases recorded fresh against `eceasy/cli-proxy-api:v7.3.4` (digest sha256:97825da3...) with the §3.4 config fragment on worker-2's stack (reference port 8387, mock port 20003; ports are masked dynamic fields per `meta.yaml` — worker port conventions vary, no assertion depends on literals). Layout per BOOTSTRAP §7: each `tests/fixtures/S2d9/S2d9-NN/` contains `meta.yaml`, `request.http` (exact bytes sent), `downstream.md` (status + raw headers + exact body bytes, plus the raw chunked stream for SSE cases), `upstream.jsonl` (what the reference emitted to the mock), `mock-response.json` (exact scripted reply). PORTS CROSS-POINTER: spec/recordings/S2d9.cases.json lists draft ports (`downstream_base_url` 8417, `reference_config_addition` mock 23001 per the worker-5-era substitution note); the ACTUAL recorded ports (reference 8387, mock 20003) are in each `meta.yaml` — recorded values win; no assertion depends on literals. S2d9-17's `meta.yaml` additionally carries the slow-chunk investigation table (as-specified / halved-sleeps / single-pause variants, all 200).
...
3. RESOLVED for observables (fixtures S2d9-11/S2d9-12): the 429 rate-limit cooldown is not disabled by `transient-error-cooldown-seconds: -1`; its window is driven by the 429 body's `resets_in_seconds`/`resets_at` (recorded: 3600 -> 1h). The no-reset-fields default is provider policy (recorded codex figure ~4s, S2d5 golden S2D5-24). The selection error carries `Retry-After` plus `reset_seconds`/`reset_time` fields (§5.1). Scheduling/cooldown algorithm internals remain S4 scope.
