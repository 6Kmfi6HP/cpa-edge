...
| `input[].id` | Normalized: NON-EMPTY ids of typed items (`message`/`reasoning`/`function_call`/`custom_tool_call`/`custom_tool_call_output`) that LACK their type prefix are re-prefixed (`msg_`, `rs_`, `fc_`, `ctc_`, `ctco_`); empty ids stay empty; already-prefixed ids pass verbatim; ids > 64 runes are deterministically shortened with a SHA-256 hash suffix (collision variants get further hash-suffixed attempts); re-prefixed ids that collide with a preserved id are rewritten to a hash-suffixed variant; encrypted reasoning items with overlong ids are dropped entirely. | `ref:internal/runtime/executor/helps/codex_input_ids.go` — `SanitizeCodexInputItemIDs`, `normalizeCodexInputItemID` |
...
| `reasoning` | CAPABILITY-DEPENDENT: for a thinking-CONFIGURED model (`codex-api-key.models[].thinking.levels` set) the object is preserved/applied (effort may be set from thinking-suffix/config). For a thinking-LESS configured model, a request that carries any reasoning config has the WHOLE `reasoning` object DELETED before forwarding — RECORDED: S2d9-05 (client sent `{"effort":"medium","summary":"auto"}`; upstream body has no `reasoning` key). User-defined (unconfigured) models: OPTIONAL effort application from suffix. | `ref:internal/thinking/apply.go` (thinking==nil + request carries config -> `StripThinkingConfig`); `ref:internal/thinking/strip.go`; `ref:internal/thinking/provider/codex/apply.go` |
...
A request is *native Lite* iff source and response formats are Responses/Codex AND (header `X-OpenAI-Internal-Codex-Responses-Lite` whose value is case-insensitively `true` after trimming, OR body `client_metadata.ws_request_header_x_openai_internal_codex_responses_lite` == JSON `true` OR the JSON string `"true"` case-insensitively) (`ref:internal/runtime/executor/helps/codex_native.go`, `ref:internal/util/codex.go` — `IsCodexResponsesLiteRequest`). Lite requests: `instructions` left untouched, `parallel_tool_calls` forced `false`, NO image-generation tool injection, Lite header forwarded upstream. All S2d9 goldens exercise the non-Lite default except S2d9-07.
...
- Downstream framing: every complete frame is emitted with a full blank-line terminator that PRESERVES the line ending it arrived with — LF frames are completed to `\n\n`; CRLF endings are preserved and extended to `\r\n\r\n` (blank upstream separator lines are not forwarded as separate chunks). A comment or `event:` line arriving without its data line is held and glued (with `\n`) onto the following frame. `ref:sdk/api/handlers/openai/openai_responses_handlers.go` — `responsesSSEFramer`, `writeResponsesSSEChunk`.
...
- Terminal success events: `response.completed`, `response.incomplete`, `response.done` — after forwarding the terminal frame the stream CLOSES (no further upstream data forwarded) and the gateway appends one final `\n` byte (`ref:sdk/api/handlers/openai/openai_responses_handlers.go` — `WriteDone`). An upstream data frame typed `response.done` is RENAMED to `response.completed` in the forwarded payload before it is emitted, so downstream data payloads never carry `type: response.done` (`ref:internal/runtime/executor/codex_websockets_errors.go` — `normalizeCodexWebsocketCompletion`, applied in the SSE stream path).
...
For non-Lite clients, if a terminal event's `response.output` is missing or empty while `response.output_item.done` events were seen, the gateway RECONSTRUCTS `response.output` from the recorded items (sorted by `output_index`, unindexed items appended in order). The EXECUTOR additionally fills missing item `id`s in an existing non-empty `response.output` from the matching `output_item.done` items. Scope of the two layers (`ref:internal/runtime/executor/codex_executor_terminal.go` — `patchCodexCompletedOutput` incl. `hydrateCodexCompletedOutputItemIDs`; `ref:sdk/api/handlers/openai/openai_responses_handlers.go` — `repairCompletedPayload`): executor = output rebuild + id hydration, applied on both stream (non-Lite) and non-stream paths; client-side framer = output rebuild ONLY (no id hydration), a defensive second pass. For native Lite clients the executor-side patch is skipped (client-side output rebuild still applies).
...
- RECORDED (S2d9-09/10/11): valid-JSON upstream error bodies are re-serialized by the gateway, NOT byte-identical: every observed field of the upstream `error` object is preserved (message, type, code, param, and passthrough fields like `resets_in_seconds`), but the wire layout is gateway-generated — alphabetical key order, compact separators, no spaces. Examples (fixtures S2d9-10/S2d9-11): upstream `{"error":{"message":"model not found: mock-codex-upstream","type":"invalid_request_error","code":"model_not_found","param":"model"}}` -> downstream `{"error":{"code":"model_not_found","message":"model not found: mock-codex-upstream","param":"model","type":"invalid_request_error"}}`; upstream 429 body -> `{"error":{"code":"usage_limit_reached","message":"You have exceeded your usage limit","resets_in_seconds":3600,"type":"usage_limit_reached"}}`. Content preservation is MODULO the gateway's error sanitizers where they apply on a given path (secret-ish keys/values redacted to `[REDACTED]`; strings truncated at 2048 runes). Because the re-marshal is deterministic (alphabetical keys, compact separators), BYTE-EXACT comparison against the fixture bytes is feasible and preferred for these bodies; structural comparison remains the fallback for upstream layouts the gateway did not already normalize. CONTRAST (intentional divergence between executors): the Claude executor passes HTTP error bytes through VERBATIM (S2d3 fixtures preserve original spacing/order).
...
- Credential side effects (cooldown policy) are S4 scope, but two RECORDED FACTS are observable through this route and pinned by goldens:
  - An upstream 429 puts the credential into a rate-limit cooldown that `transient-error-cooldown-seconds: -1` does NOT disable. The cooldown window length comes from the 429 body's `resets_in_seconds` / `resets_at` (S2d9-11 declared 3600 -> 1h). When the body carries no reset fields the default is provider-policy (recorded codex figure: ~4s, S2d5 golden S2D5-24); the algorithm is S4 scope. An immediate repeat during the window fails credential selection with NO upstream call; downstream gets 429 with header `Retry-After: <reset_seconds>` and body `{"error":{"code":"model_cooldown","last_upstream_error":"<upstream error.type>: <upstream error.message>","message":"All credentials for model <m> are cooling down via provider codex (last error: <same summary>)","model":"<m>","provider":"codex","reset_seconds":<n>,"reset_time":"<Go duration string>"}}` — recorded verbatim in fixture S2d9-12. The direct 429 response (S2d9-11) carries NO `Retry-After` header; only the cooldown selection error does.
  - RECORDED (fixture S2d9-18): an upstream 404 `model_not_found` ALSO puts the credential into a cooldown that survives `transient-error-cooldown-seconds: -1` and outlasts multi-second waits. During the window, a repeat of the same request fails credential selection with NO upstream call and returns HTTP 503 with body `{"error":{"message":"auth_unavailable: no auth available (providers=codex, model=codex-mock; last upstream error: model_not_found: model not found: mock-codex-upstream)","type":"server_error","code":"internal_server_error"}}` — the 503 `auth_unavailable` selection-error family (gateway struct layout: message, type, code), NOT the 429 `model_cooldown` family of the §5.1 first bullet. Duration policy is S4 scope; S2d9-18 pins only the observable pair (trigger 404 -> window 503, zero upstream calls).
...
- The `error` detail object, in recorded order of preference: (1) if the upstream error body was a JSON object with an `error` (or `response.error`) object, that object is sanitized (secret-ish keys redacted `[REDACTED]`, strings truncated at 2048 runes) and used verbatim as the detail; (2) if there is no nested error object but the upstream error frame carries its OWN error fields (top-level `code`/`message`/`error_type`/`param`), the detail is built from THOSE fields verbatim — RECORDED: S2d9-13/S2d9-14 detail == `{"code":"server_error","message":"upstream exploded"}` exactly as declared by the frame (no type/param added); (3) only when the upstream supplies no error fields at all is `{type, code, message, param}` synthesized from the mapped status (401->invalid_api_key, 403->insufficient_quota, 429->rate_limit_exceeded, 404->model_not_found, 408->request_timeout, >=500->internal_server_error, other 4xx->invalid_request_error; type server_error for >=500 else invalid_request_error) (`ref:sdk/api/handlers/openai_responses_stream_error.go`; sanitizers in `openai_responses_handlers.go`). On the wire the detail object is serialized with alphabetical keys (recorded S2d9-08: `{"code":"request_timeout","message":"...","param":null,"type":"invalid_request_error"}`).
...
- Request: near-verbatim (NO codex translator runs): `stream` deleted, `model` rewritten per §3.2, `instructions` defaulted per §3.2 when absent, reasoning-item sanitization (§3.2) applied, tool JSON schemas normalized (§3.2 OPTIONAL rules), `parallel_tool_calls` normalized per §3.2, and the SAME derived `prompt_cache_key` + `Session-Id` header attachment as §3.2 applies (recorded: fixture S2d9-15 upstream body carries the derived cache key); NO `store` forcing, NO `include` forcing, NO `previous_response_id` deletion, NO image-generation injection (`ref:internal/runtime/executor/codex_executor_execute.go` — `executeCompact`: target format `openai-response`, i.e. registry passthrough; same `cacheHelper` as `/responses`).
...
RECORDED COMPLETE (2026-09-15/16, oracle worker-2, single provenance; worker-1's superseded cancelled run wiped): all 17 cases recorded fresh against `eceasy/cli-proxy-api:v7.3.4` (digest sha256:97825da3...) with the §3.4 config fragment on worker-2's stack (reference port 8387, mock port 20003; ports are masked dynamic fields per `meta.yaml` — worker port conventions vary, no assertion depends on literals). Layout per BOOTSTRAP §7: each `tests/fixtures/S2d9/S2d9-NN/` contains `meta.yaml`, `request.http` (exact bytes sent), `downstream.md` (status + raw headers + exact body bytes, plus the raw chunked stream for SSE cases), `upstream.jsonl` (what the reference emitted to the mock), `mock-response.json` (exact scripted reply). PORTS CROSS-POINTER: spec/recordings/S2d9.cases.json lists draft ports (`downstream_base_url` 8417, `reference_config_addition` mock 23001 per the worker-5-era substitution note); the ACTUAL recorded ports (reference 8387, mock 20003) are in each `meta.yaml` — recorded values win; no assertion depends on literals. S2d9-17's `meta.yaml` additionally carries the slow-chunk investigation table (as-specified / halved-sleeps / single-pause variants, all 200). A supplementary 18th fixture, S2d9-18, pins the 404->cooldown-window observation requested by gate round 1 (N11): it is not one of the 17 contract cases but a recorded observation pair (see §5.1).
...
| S2d9-18 | tests/fixtures/S2d9/S2d9-18/ | 404->cooldown-window observation: repeat of the S2d9-10 request -> 503 auth_unavailable selection error, zero upstream calls (N11 pin, gate round 1) | upstream-error (sequenced) |
...
2. RESOLVED (fixture S2d9-08): the Responses-client disconnect payload is the source-derived 408/`request_timeout` shape — §5.3 now carries the exact recorded bytes. The `unexpected EOF`/`internal_server_error` variant from the orchestrator wire notes belongs to the gemini executor family; no S2d9 amendment was needed beyond confirmation.
3. RESOLVED for observables: TWO distinct recorded cooldown families, both surviving `transient-error-cooldown-seconds: -1`: (a) rate-limit — upstream 429, window driven by the body's `resets_in_seconds`/`resets_at` (recorded: 3600 -> 1h; no-reset default is provider policy, recorded codex figure ~4s, S2d5 golden S2D5-24), window response = 429 `model_cooldown` + `Retry-After` (fixtures S2d9-11/S2d9-12); (b) model-not-found — upstream 404 `model_not_found`, window response = 503 `auth_unavailable` selection error with the last upstream error embedded (fixture S2d9-18). Scheduling/cooldown algorithm internals remain S4 scope.
