...
| POST | `/v1/images/generations` | 200 | 400 (non-JSON body, missing `prompt`, unsupported model); 404 when `disable-image-generation: all` |
| POST | `/v1/images/edits` | 200 | same class as generations |
| POST | `/v1/videos` | 200 | 400/5xx (xAI videos create) |
| POST | `/v1/videos/generations` | 200 | " |
| POST | `/v1/videos/edits` | 200 | " |
| POST | `/v1/videos/extensions` | 200 | " |
...
| GET | `/v1/live/:call_id` | 101 (WS) | Sideband WS; non-WS → 426; unknown call id → 404 `{"error":"Codex live session not found"}`; claimed by another connection → 409. Evidence: `internal/client/codex/live/sideband.go` `HandleSideband` |
...
|---|---|---|---|
| GET | `/v1/realtime` | realtime | 101 (WS); with `?call_id=` behaves as sideband; **non-WS request → 426** `Upgrade: websocket` header + body `{"error":{"message":"WebSocket upgrade required","param":null,"type":"invalid_request_error","code":"websocket_upgrade_required"}}` (map-sorted keys). Evidence: `internal/client/codex/live/websocket.go` `HandleRealtimeWebsocket`/`HandleDirectWebsocket`. |
| POST | `/v1/realtime` | realtime | 200 (calls API) — same `Handle` as /v1/live |
| POST | `/v1/realtime/calls` | realtime | 200 |
| GET | `/v1/realtime/calls/:call_id` | realtime | 101 (WS sideband) |
| POST | `/v1/realtime/calls/:call_id/hangup` | standard | SIP control |
| POST | `/v1/realtime/calls/:call_id/accept` | standard | " |
| POST | `/v1/realtime/calls/:call_id/reject` | standard | " |
| POST | `/v1/realtime/calls/:call_id/refer` | standard | " |
| POST | `/v1/realtime/client_secrets` | standard | 200 (client-secret mint; bad JSON → 400 `"Invalid Realtime client secret request"`) |
| POST | `/v1/realtime/sessions` | standard | 200 (legacy session credential) |
| POST | `/v1/realtime/transcription_sessions` | standard | 200 |
| GET | `/v1/realtime/translations` | realtime | 200 / WS |
| POST | `/v1/realtime/translations` | realtime | 200 |
| POST | `/v1/realtime/translations/client_secrets` | standard | 200 |

Evidence: `internal/api/server_routes.go` (realtime block). Error envelope on realtime auth failures differs from the rest of the client API — §4.2.
...
- Unknown method on a known management path → 404 empty (R-404); unknown `/v0/management/<path>` → 404 empty (probe 15), except when a plugin route matches (plugin host dispatch).
- `GET|POST /v0/management/oauth-callback` sits **outside** the management-key middleware (only availability middleware): 400 `{"error":"<message>","status":"error"}` with message one of `state is required`, `invalid state`, `code or error is required`, `unsupported provider`, `provider does not match state`, `invalid redirect_url`; 404 `{"error":"unknown or expired state","status":"error"}` (recorded S1-20); 409 `{"error":"<oauth flow is already completed|session status|oauth flow is not pending>","status":"error"}`; 200 `{"status":"ok"}`; 500 `{"error":"<handler not initialized|failed to persist oauth callback>","status":"error"}`. Map-sorted wire order puts `error` before `status` (§2); only `{"status":"ok"}` keeps its literal order. Evidence: `internal/api/server_management.go` route block; `internal/api/handlers/management/oauth_callback.go`; fixture S1-20 (mgmt-callback-badstate).
...
- **Example-API-key safe mode** (OPTIONAL): when `api-keys` contains upstream template values, `GET|HEAD /` and `/management.html` serve an HTML warning page, and all proxy paths under `/v1`, `/v1beta`, `/openai/v1`, `/backend-api/codex` return 403 `{"error":"unsafe_example_api_key","message":"Proxy API endpoints are disabled because api-keys contains template values. Open /management.html?safe-mode=configure, update api-keys in Management, then retry."}` + `X-CPA-SAFE-MODE: example-api-key` header. Evidence: `internal/api/server_middleware.go` `exampleAPIKeySafeModeMiddleware`. Not golden-recordable with the standard oracle config (needs a template-keys config run) — see OQ-5.
...
Config-conditional MUST: when the configured `api-keys` list is empty, the config-key provider is not registered and **all client routes are open** (no 401 is ever produced by this surface). Evidence: `internal/access/config_access/provider.go` `Register` unregisters on empty; `internal/api/server_middleware.go` treats provider-success as pass.
...
- Standard-auth failures on realtime routes return a **different envelope** (OpenAI-nested): `401 {"error":{"message":"Missing API key"|"Invalid API key","param":null,"type":"authentication_error","code":"invalid_api_key"}}` (map-sorted keys); a broken presented client secret → `401 {"error":{"message":<reason>,"param":null,"type":"invalid_request_error","code":"invalid_realtime_client_secret"}}`; auth-service internal errors → `{"type":"server_error","code":"authentication_service_error"}`. Evidence: `internal/api/server_middleware.go` `realtimeStandardAuthMiddleware`, `realtimeAuthMiddleware`.
...
- Reject/accept matrix:
  ...
  - ≥5 failed attempts from one client IP → `403 {"error":"IP banned due to too many failed attempts. Try again in <duration>"}` for the ban duration (30 min)
...
- **Trailing slash (recorded, S1-08):** gin's `RedirectTrailingSlash` is on by default and the redirect is emitted by the router layer BEFORE the middleware chain, so:
...
- **Request body decoding:** handlers reading JSON accept bodies with `Content-Encoding: zstd` (decoded first; on decode failure with invalid JSON → 400 `"Invalid request: ..."`; valid JSON passes through). Unsupported content encodings → 400. Evidence: `sdk/api/handlers/request_body.go` `ReadRequestBody`.
...
3. **OpenAI-shaped request/server errors:** `{"error":{"message":"<msg>","type":"<invalid_request_error|authentication_error|permission_error|rate_limit_error|server_error>","code":"<code>","param":"<param>"}}` — `param` and `retryable` optional. `model_not_found` literal (before enrichment): `{"error":{"message":"unknown provider for model <model>","type":"invalid_request_error","code":"model_not_found","param":"model"}}` (status 400).
...
6. **Live/alpha-search errors:** `{"error":"<message>"}` (status 503/400/413/426 as per §3.2/3.7). Status/message source: `internal/client/codex/live/live.go` `writeLiveError`.
7. **Realtime non-WS 426:** see §3.7 (with `Upgrade: websocket` response header).
...
| Realtime route without WS upgrade | 426 | §3.7 JSON |
...
Case definitions: `spec/recordings/S1.cases.json` (24 recordable cases + 5 FIXTURE-DEFERRED). **All 24 cases recorded 2026-09-15** by @oracle-runner against CLIProxyAPI v7.3.4 (image digest sha256:97825da3...) — 24 fixture dirs, 203 files under `tests/fixtures/S1/`. Layout per RECIPES (`reports/oracle/BOOTSTRAP.md` §7): per case `meta.yaml` (anchor, config, dynamic_fields, per-request http_status), one `<request>.request.http` + `<request>.downstream.md` per request, `upstream.jsonl` + `mock-response.json` for provider-attached cases. Dynamic fields masked by the contract layer: `Date`, `X-Cpa-Trace-Id`, `created`, `created_at`.
...
- **OQ-5 (safe mode):** example-API-key safe-mode page + 403 gate is spec'd (§3.10) but not golden-recordable with the standard oracle config; needs a dedicated config run — deferred, not dropped.
