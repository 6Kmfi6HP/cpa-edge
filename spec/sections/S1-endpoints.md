...
The gate is shared verbatim by every client group: S1-26 (recorded) pins all five transports on the Claude route `POST /v1/messages` itself — `?key=`, `?auth_token=`, `X-Goog-Api-Key`, and a **non-Bearer `Authorization` value compared verbatim** (e.g. `Authorization: oracle-local-key-1` with no scheme) all pass the gate (each probe reaches routing and fails with the 400 Claude-shaped `model_not_found`); a non-Bearer **scheme** (`Authorization: Basic <valid key>`) makes the whole header value the candidate and is rejected with 401 `{"error":"Invalid API key"}`. Together with S1-07 (four transports on `GET /v1/models`, `X-Api-Key` on `POST /v1/messages`), the transport matrix is fully covered per route group.
...
4. **Claude-shaped errors** (`/v1/messages`, `/v1/messages/count_tokens`): `{"type":"error","error":{"type":"<api_error|invalid_request_error|authentication_error|billing_error|permission_error|not_found_error|request_too_large|rate_limit_error|timeout_error|overloaded_error>","message":"<msg>"}}` (struct order: type, message; recorded `Content-Type: application/json` — NO charset suffix, unlike the auth-401 `application/json; charset=utf-8`; fixtures S1-11/S1-26). A `model_not_found` upstream of this renderer collapses to `{"type":"error","error":{"type":"invalid_request_error","message":"unknown provider for model <model>"}}`.
...
Case definitions: `spec/recordings/S1.cases.json` (26 recordable cases — S1-01…S1-24, the gate round-1 follow-up batch S1-25, and the cla2gem cross-check S1-26 — and 5 FIXTURE-DEFERRED, one of which, S1-D3, is now closed by S1-25). **All 26 cases recorded 2026-09-15** by @oracle-runner against CLIProxyAPI v7.3.4 (image digest sha256:97825da3...) — 26 fixture dirs, 294 files under `tests/fixtures/S1/` (203 + 80 in S1-25 + 11 in S1-26). Layout per RECIPES (`reports/oracle/BOOTSTRAP.md` §7): per case `meta.yaml` (anchor, config, dynamic_fields, per-request http_status; S1-25 additionally records the config variant per request), one `<request>.request.http` + `<request>.downstream.md` per request, `upstream.jsonl` + `mock-response.json` for provider-attached cases. Dynamic fields masked by the contract layer: `Date`, `X-Cpa-Trace-Id`, `created`, `created_at`, ban-remaining durations.
...
| S1-26 | cla2gem cross-check (impl-review N6): all five credential transports on POST /v1/messages — ?key=, ?auth_token=, X-Goog-Api-Key, non-Bearer verbatim Authorization (all pass the gate → 400 Claude-shaped model_not_found); Basic-scheme rejection → 401 Invalid API key; also pins the Claude renderer's `Content-Type: application/json` (no charset) | 11 | recorded |
