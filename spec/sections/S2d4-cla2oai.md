...
- Non-stream success: status 200, `Content-Type: application/json` (bare, no charset — set by the Claude handler), plus the global CORS block (S1) and possibly `X-CPA-TRACE-ID` (dynamic).
- Content-Type distinction (recorded): middleware-level responses (the two 401s) carry `Content-Type: application/json; charset=utf-8` (gin's JSON renderer); every handler-produced response — success, error, SSE — uses the bare `application/json` / `text/event-stream` forms above.
...
| `thinking` | `reasoning_effort` | Two-stage; the EFFECTIVE mapping table below is the contract. Absent `thinking` → no `reasoning_effort`. |
...
- `role: "system"` INSIDE `messages` is not a real Anthropic role but the gateway accepts it: its text content (string or text blocks, non-empty, non-attribution) becomes a `{"role":"user","content":[{"type":"text","text":...}]}` message whose text is wrapped as `<system-reminder>\n<text>\n</system-reminder>` (one wrapper for all its text parts joined by `\n`). Non-text-only system messages are dropped. Ordering: a reminder arriving while tool_use ids are pending is buffered and flushed by the NEXT message that produces output — AFTER that message's tool-result messages when it carries tool_results, otherwise immediately BEFORE that message's own content message; buffered reminders never re-order across a tool-result batch, and trailing buffered reminders (nothing follows) are appended at the end of `messages`.
- Content part types:
...
- Assistant message shape: a SINGLE upstream `{"role":"assistant", ...}` message carrying `"content"` (array of surviving text/image parts, or the empty string `""` when none), `"reasoning_content"` (see below) and `"tool_calls"` (see below) — never split into multiple assistant messages. EMISSION GATE: an assistant message is emitted only when at least one of content parts, reasoning_content, or tool_calls survived; an assistant message where nothing survives (e.g. only a dropped empty-signature thinking block) produces NO upstream message at all. Non-assistant messages use the narrower gate of surviving content parts only.
...
- `tool_result` handling: each `tool_result` block becomes ONE upstream message `{"role":"tool","tool_call_id":<tool_use_id>,"content":<string>}`. Content string construction: array items that are strings or `{"type":"text"}` contribute their text; image items are extracted for relay (below); other items contribute their raw JSON; texts joined with `\n\n`. Edge cases (all testable): joined text empty AND images extracted → content is the exact placeholder `[Tool returned image content; the images follow in the next user message.]`; joined text empty and NO images → content is the RAW JSON of the block's `content` field (e.g. `[{"type":"text","text":" "}]` verbatim); `content` an EMPTY ARRAY → content is the string `[]`; `content` absent entirely → `""`.
...
- Tools array: each `{"name","description","input_schema"}` → `{"type":"function","function":{"name","description","parameters"}}`:
  - `input_schema` is RE-SERIALIZED — sjson delegates non-raw values to Go `encoding/json`, which emits sorted map keys and HTML-escapes `<`,`>`,`&` — after schema normalization: `type:"object"` without `properties` GAINS `"properties":{}`; `pattern` values and `patternProperties` keys using unsupported unicode property escapes are deleted recursively (evidence: `normalizeObjectSchemaProperties` + `internal/util` schema keyword lists).
...
3. Tool calls: for each `delta.tool_calls[i]` the id (string, non-empty) and function name are recorded until the block start is emitted; a `content_block_start` (tool_use) is emitted MID-STREAM as soon as the accumulator has a non-empty id AND a non-empty name AND no other tool block is open. The emitted `content_block.id` is the SANITIZED id (same SanitizeClaudeToolID rule as §3.4 — the streaming path sanitizes too). The name is tool-name-mapped as in §3.4. `function.arguments` deltas are accumulated but NOT forwarded incrementally: the complete arguments (after `FixJSON`) are emitted as ONE `input_json_delta` at the block's finalize, immediately followed by `content_block_stop`; when NOTHING accumulated, NO `input_json_delta` is emitted (start+stop only). A finish_reason frame (or [DONE]) finalizes ALL tool blocks in DETERMINISTIC ASCENDING OpenAI-index order, emitting any belated starts: a tool call that never got a name but has id and/or arguments gets a synthesized name `tool_<openai-index>` (OPTIONAL defensive behavior; cited, unrecorded).
...
- `<EST>` = o200k_base token count of the trimmed non-empty segments of the ORIGINAL client request, joined with `\n`. The segment collector in `claude_input_tokens.go` (`collectClaude*TokenSegments`) is NORMATIVE for this estimate: it walks the client body independently of the request translation, so part types the translator DROPS are still COUNTED here (e.g. `search_result`/`web_search_result`, `web_fetch_result`, `code_execution_result` family, `tool_reference`, generic objects' `text` fields), while `image`/`audio`/`video`/`redacted_thinking` contribute nothing. Concretely: system texts (string or text blocks); per message: the `role` string and the content's text segments (text blocks, thinking blocks' `thinking`, document title/context and text-source `data`/`content`, tool_use id+name+compact-input, tool_result tool_use_id+tool_call_id+content recursion); tools (`type`, `name`, `description`, compact `input_schema`); `tool_choice` (string or type+name).
...
- `upstream.jsonl` entries are compared deep-equal (JSON), arrays order-sensitive; object key order is informative (the reference's emission order is documented in §3.1) but not contract-pinned; headers compared semantically (presence/absence/value where stable). The wire log REDACTS secret-bearing headers (`Authorization`, `x-goog-api-key`, `x-api-key`) to `<redacted>`, so header comparison for those asserts PRESENCE only — never the bearer value.
...
8. **Last-error summary formatting** — mechanism (evidence: `sdk/cliproxy/auth/selector.go` `ExtractUpstreamErrorSummary`): the summarizer looks for the byte sequence `": {"` within the first 50 bytes of the error text; when found, everything after it is taken as the JSON candidate. A VALID candidate (or no cut firing at all) yields the compact `<code>: <message>` form; an INVALID candidate (the cut mangled the JSON) makes the summarizer return the sanitized RAW error text VERBATIM. The recorded golden (S2d4-cooldown-second) hits the second branch — the mock's spaced body `{"error": {"message": …}}` is cut at `"error": {"`, leaving a trailing-brace fragment that fails JSON validation, so the raw body is embedded verbatim. An upstream serving the same error compactly (no space after the colon) would surface the compact form; that branch is not golden-pinned here.
