...
- Assistant message shape: a SINGLE upstream `{"role":"assistant", ...}` message carrying `"content"` (array of surviving text/image parts, or the empty string `""` when none), `"reasoning_content"` (see below) and `"tool_calls"` (see below) — never split into multiple assistant messages.
  - `tool_calls` entry per `tool_use` block: `{"id":<block id verbatim>,"type":"function","function":{"name":<name>,"arguments":<input RAW bytes as a JSON string>}}`; `input` absent → `"{}"`.
...
- The upstream is always requested with `stream:true` semantics for this client mode (client asked to stream). Frames are parsed line-wise: `data:` lines are collected (multiple `data:` lines in one frame are joined with `\n`); `event:` lines are captured as the frame's event name; `:`/`id:`/`retry:` lines are ignored. A blank line ends the frame.
...
1. `message_start` is emitted on the FIRST chunk that has a `choices[0].delta` object — regardless of whether the delta carries a `role` field. Its `id`/`model` come from that chunk; later chunks never update them. `usage.input_tokens` is the local estimate `<EST>` (§4.3); `output_tokens` is 0.
...
4. Text/thinking deltas arriving while a tool_use block is open are buffered (not emitted inline) and flushed at finalize as complete start/delta/stop triples after the tool blocks, in arrival order, with fresh indexes.
5. `finish_reason` is captured from `choices[0].finish_reason` when a non-empty string: `length` and `content_filter` are kept verbatim in the internal state; with an announced tool block (`content_block_start` emitted): valid-JSON-arguments → `tool_calls`, invalid/empty arguments → `length`; `tool_calls` WITHOUT any announced tool block → `stop`; anything else kept verbatim. The capture frame also finalizes all open blocks (stops text/thinking; flushes tool blocks per rule 3/4).
6. Usage capture: whenever a chunk has a non-null `usage`, its values are cached. `message_delta` + `message_stop` are emitted at the EARLIEST of: (a) a trailing usage-only chunk (a `usage` present, no `choices[0]`, and something was already generated) — both events then; or (b) `[DONE]` (synthesized or real). `[DONE]` emits only what is still missing: finalize blocks → `message_delta` (if not yet sent) → `message_stop` (if not yet sent).
...
| Immediate retry while the credential is in the ~1s rate-limit cooldown | recorded (expected 429) | `{"type":"error","error":{"type":"rate_limit_error","message":"All credentials for model <model> are cooling down via provider <provider> (last error: <code>: <upstream message>)}}` — the internal OpenAI-shaped model_cooldown payload is reshaped to the Claude type/message pair. `transient-error-cooldown-seconds: -1` does NOT disable this cooldown **[R]** | S2d4-cooldown-second |
...
Recordings: CLIProxyAPI v7.3.4 (docker image digest `sha256:97825da3009f98acf78b5c172fde650a5fbe7a690950a69ce6d7b535d77d4266`), oracle env per `reports/oracle/BOOTSTRAP.md` §3 + §7 RECIPES; mission recordings requested via `spec/recordings/S2d4.cases.json`. All S2d4 behaviors are RECORDABLE-LOCALLY (openai-compatibility provider with base-url override per R-FIXTURE): fixtures recorded by @oracle-runner-5 against the fleet mock (`mock_openai.py`, openai wire).

Recording environment (oracle-runner-5 stack): reference on `127.0.0.1:8417`, openai mock on port `22999` (base-url `http://host.docker.internal:22999/v1`); client api key `oracle-local-key-1`; upstream provider key `mock-upstream-key`; alias `mock-model` → upstream model `mock-gpt-model`; plus a second provider entry `mock-openai-compat` (alias `mock-model-compat`, `is-compat: true`) for S2d4-compat-thinking. Port numbers anywhere in a transcript are masked dynamic fields; all other bytes are compared exactly (§4.5).

Fixtures live in `tests/fixtures/S2d4/<case-id>/` per the RECIPES layout (`meta.yaml`, `request.http`, `downstream.md`, `upstream.jsonl`, `mock-response.json`).
...
RECORDING STATUS: 22 cases requested from @oracle-runner-5 (message sent; fixtures pending at section submission time — this table lists the requested fixture set; see `spec/recordings/S2d4.cases.json` for the full request including run-order constraints). Deferred: none (all cases RECORDABLE-LOCALLY).
...
2. **Cooldown second-request status** — derived as 429 (status unwrap chain through the stored last error); the golden records the truth. If the reference answers 500 instead, §5.2's expected-status cell must be corrected from the fixture, not from this text.
3. **`force-mapping: true`** response model rewrite and **provider `headers:`** custom-header injection are config-dependent OPTIONAL behaviors (documented §2.2/§3.3, unrecorded). Recording them needs a config variant similar to S2d4-compat-thinking; deferred by mutual agreement unless the implementer needs them.
4. **Multiple choices** (`choices` length > 1): only `choices[0]` is translated (§3.4). The reference behavior is pinned by code reading; an upstream sending 2+ choices is out of the golden set.
5. **`toolu_` empty-id generation** (§3.4) embeds wall-clock time — dynamic; the golden avoids empty ids by construction.
6. **In-stream keep-alive comments** (`: keep-alive`) are emitted only when `streaming.keep-alive-seconds` is configured (> 0); the oracle config has it disabled and goldens assert no comment frames. OPTIONAL otherwise.
7. Non-equivalence (accepted): the reference's non-stream gzip magic-byte decompression (§2.2) is a Claude-handler quirk kept for compatibility; CPA-Edge reproduces it as-is.
