...
| `plugins.store-auth` | [] of `{match, apply-to: [registry, artifact], type: bearer, token-env}` | (empty) | store-registry auth rules; token values are read from environment variables and never written into plugin manifests or node status (`sdk/pluginstore` auth config) |
