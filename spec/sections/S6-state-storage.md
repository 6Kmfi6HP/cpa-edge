...
| OAuth callback handshakes | document | namespace `oauth-callbacks`, key = `.oauth-<provider>-<state>.oauth` (upstream writes these handshake files into the auth dir; content `{code?, error?, state}`) | `OAuthCallbackNote` = flat string map | `put()` on callback receive; `delete()` on consume — the login worker polls for the key (upstream: 500 ms poll, 5 min deadline; `auth_files_provider_oauth.go`) |
| Auth refresh bookkeeping | document | namespace `auth-refresh`, key = auth-file name | `AuthRefreshDocument` (`next_refresh_after`, `last_refreshed_at`, `status`) | `update()` callback — next-refresh scheduling and last-refreshed/status bookkeeping ride the document, not the token payload |
...
/** Mgmt auth failures (§3.3.3): per-IP counters. Upstream sweeps hourly and removes
 *  entries idle > 2 h whose ban (if any) has expired — the sweep needs last_seen.
 *  Timestamps are RFC3339 seconds-precision UTC strings. */
 ...
 *  status encoding (I-auth): `'pending'` | `<error message>`; completion is the
 *  `completed` boolean — together they form the pending/error/completed tri-state S3 reads.
 *  All timestamps (created_at, expires_at) are RFC3339 seconds-precision UTC strings. */
...
All management endpoints require the management key: `Authorization: Bearer <key>` OR `X-Management-Key: <key>` (upstream `internal/api/handlers/management/handler.go`, `Middleware()`). **Scope split with S5**: the full management surface — config-WRITING list endpoints (`PUT/PATCH/DELETE /v0/management/{api-keys, gemini-api-key, claude-api-key, codex-api-key, xai-api-key, meta-api-key, openai-compatibility, vertex-api-key, oauth-excluded-models, oauth-model-alias, oauth-request-scoped-errors, …}`), OAuth login endpoints, quota endpoints, and the auth-state readers beyond the storage-relevant shapes here — is specified and golden-recorded in S5; S6 specifies the scalar-field persistence subset (B4), the state shapes behind these endpoints, and S6-owns the fixtures listed in §6. Auth gate semantics: §3.3.3. All responses carry the global CORS block and `X-CPA-VERSION`, `X-CPA-COMMIT`, `X-CPA-BUILD-DATE`, `X-CPA-SUPPORT-PLUGIN` headers (mgmt middleware sets them; recorded in `reports/oracle/BOOTSTRAP.md` §4). Management JSON bodies built from Go maps (`gin.H`, metadata maps) serialize keys **alphabetically** — recorded across all S6/S5 fixtures; contract tests rely on it.
...
| B7 | App logs read | GET `/v0/management/logs?cursor=&after=&limit=` | 200 `{lines, line-count, latest-timestamp, next-cursor[, cursor-reset]}`; **400 `{"error":"logging to file disabled"}` when `logging-to-file: false`**; **400 `{"error":"invalid limit: must be a positive integer"}` for an unparsable `limit` (recorded, S6-13 step 3), and `{"error":"invalid limit: must be greater than zero"}` for `limit <= 0` (source-verified, `parseLimit`)**; 500 on IO errors | `management/logs.go` `GetLogs`, `parseLimit` |
...
| `plugins.store-auth` | [] of AuthConfig | (empty) | auth rules for plugin-store requests (shape: `sdk/pluginstore` AuthConfig — `match`, `apply-to`, `type` (not restricted to bearer), `token-env`, `username-env`, `password-env`, `header-name`, `header-value-env`, `allow-insecure`); secret values are read from environment variables and never written into plugin manifests or node status |
| `plugins.auth-revision` | int64 | 0 | bumped when Home-managed plugin credentials change (ignored outside Home mode) |
...
| `payload` | {`default`[], `default-raw`[], `override`[], `override-raw`[], `filter`[]} of rules `{models: [{name (wildcards allowed), protocol, headers (wildcard match-all), from-protocol, match, not-match, exist, not-exist}], params: map<JSON-path, value>}` (`filter`: `params` = JSON paths to REMOVE) | (empty) | paths use gjson/sjson syntax; `*-raw` rules whose params are not valid JSON are **dropped with a warn log** (`SanitizePayloadRules`) |
...
**Load failure semantics** (B20): with `LoadConfig` (server startup) any YAML parse error, credential-weight error, `credential-in-flight` validation error, `codex.live-media-relay` validation error, or plugin-directory resolution failure (when `plugins.enabled`) aborts startup with an error. With `LoadConfigOptional` (cloud standby) a missing/empty/invalid file yields an empty config carrying only `credential-in-flight` defaults plus normalized plugin config (`NormalizePluginsConfig`). Evidence: `config_load.go`.

### 3.2 Effective-config JSON view — GET /v0/management/config

Serialization = the effective `Config` with its JSON tags. MUST omit (`json:"-"`): `host`, `port`, `remote-management` (incl. secret-key), `auth-dir`, home config. Fields tagged `omitempty` are omitted whenever empty (optional sub-keys appear only when populated — e.g. a provider entry's `base-url` only when set); zero-valued non-omitempty fields (bools `false`, ints `0`, empty strings/lists/maps) DO appear. MUST include: `api-keys` (plaintext), all provider blocks with their `api-key` values, sanitized/normalized values (post-§3.1 defaults). Byte-shape is fixture-recorded (golden S6-01). Evidence: `config.go` struct tags; `config_basic.go` GetConfig returns a copy of the in-memory config.
...
**Registered encodings and techniques (I-auth implementation, post-admission):**
- `status` encodes `'pending'` or the verbatim error message; `completed` carries the terminal success state — S3 reads the pending/error/completed tri-state from the pair.
- **Invisible-tombstone technique** for cancel-racing transitions: because `Store.update()` callbacks are side-effect-free and re-run under contention, a cancel (DELETE `/v0/management/oauth-session`) races a completion by committing a tombstone marker through `update()`; the losing side's callback re-runs against the tombstoned document and abandons (returns it unchanged), so side effects (token persistence) happen only after the winning commit. Documented in `packages/auth` oauth-sessions implementation.
- Timestamps (`created_at`, `expires_at`, `banned_until`) are RFC3339 seconds-precision UTC strings.
### 3.4 Auth-file on-disk schema (recorded from container disk; auth dir default `/root/.cli-proxy-api` in the image)

Loading rules (evidence `sdk/auth/filestore.go` `readAuthFiles`, `internal/watcher/synthesizer/file.go`): only non-directory `*.json` files (case-insensitive suffix); empty files skipped; JSON-parse failures skipped (warn); `type` field (string) lowercased+trimmed selects the provider; `type: "gemini"` is remapped to `gemini-cli` and then **skipped by built-in synthesis** (v7.3.4 handles Gemini OAuth through plugin providers); upstream skips missing/empty `type` entirely. **CPA-Edge reconciliation (registered, I-auth)**: a missing/empty `type` parses to provider `"unknown"` — the file loads for management display (auth-files listing) but synthesizes no credential and no executor, which reconciles S3 §3.2 with this skip rule. Auth ID = relative path under auth dir (lowercased on Windows).
...
#### 3.4.2 Per-type token schemas (JSON field names are public interface; fields suffixed `?` are `omitempty` — absent when empty; all others always serialize; §3.4.1 common keys may co-appear with any type)
...
`next_retry_after`, `quota` (both levels), `code` and `retryable` are `omitempty` — the recorded golden S6-09 omits all four (no cooldown metadata on a fresh failure).
```
...
- Query: `cursor` (opaque), `after` (unix seconds cutoff), `limit` (positive int). Unparsable `limit` ⇒ 400 `{"error":"invalid limit: must be a positive integer"}` (recorded, S6-13 step 3); `limit <= 0` ⇒ 400 `{"error":"invalid limit: must be greater than zero"}` (source-verified: `logs.go` `parseLimit`; no golden).
...
MUST: the runtime keeps the most recent application log entries in Store ring `logs`, capacity **1000** (`DEFAULT_RING_CAPACITY`; this exact capacity is a **CPA-Edge design choice** — upstream keeps no general log ring, only the 1024-entry Home log-forwarder queue), one `LogRingEntry` per emitted line; `level` is the closed set `'trace'|'debug'|'info'|'warn'|'error'|'fatal'|'panic'` (the formatter renders logrus `warning` as `warn`):
...
2. **Availability cache**: `GetAvailableModels(<protocol>)` results cached per protocol and invalidated on any registration/unregistration/quota change (generation counter). Evidence: `internal/registry/model_registry.go` (`availableModelsCache`, `invalidateAvailableModelsCacheLocked`). The auxiliary catalogs serve narrower consumers: `codex_client_models.json` backs the `client_version` query parameter path of `/v1/models` (`sdk/api/handlers/openai/openai_handlers.go` `codexClientModelsResponse`), and `devin_models.json` backs Devin executor model resolution (`internal/registry/devin_models.go`). CPA-Edge: derived in-memory; catalog documents cached in Store `models/catalog:<name>`; nothing else persisted.
...
- Nil-handler vs missing-dependency are distinct upstream shapes (do not merge): a nil management handler answers **500 `{"error":"handler unavailable"}`** (e.g. logs.go handlers when `h == nil`); an absent core auth manager answers **503 `{"error":"core auth manager unavailable"}`** (auth-files upload/delete/patch endpoints). CPA-Edge maps both to error code `unavailable`, preserving the HTTP status split.
