...
- A body that is not valid JSON is NOT rejected by a JSON-syntax check: the handler reads it, `model` resolves to `""` and the request fails with the same 400 `model_not_found` (evidence: `openai_responses_handlers.go` `Responses` → `handlers.ReadRequestBody` performs no JSON validation; `request_body.go`).
...
- `usage`: mapped from the chat `usage` when any of `prompt_tokens`/`completion_tokens`/`total_tokens` exists: `usage.input_tokens` = prompt_tokens, `usage.input_tokens_details.cached_tokens` (only when upstream has `prompt_tokens_details.cached_tokens`), `usage.output_tokens` = completion_tokens, `usage.output_tokens_details.reasoning_tokens` (only when upstream has it), `usage.total_tokens` = total_tokens — all keys in that order. If the chat usage has NONE of the three base fields, the whole upstream `usage` object is copied verbatim instead.
...
Stream `response.completed/incomplete` usage key order: `input_tokens`, `input_tokens_details.cached_tokens`, `output_tokens`, (`output_tokens_details.reasoning_tokens` only when > 0), `total_tokens`. The stream path does NOT run `EnsureResponsesUsageDetails`, so a plain `{prompt_tokens,completion_tokens,total_tokens}` upstream yields `{"input_tokens":9,"input_tokens_details":{"cached_tokens":0},"output_tokens":6,"total_tokens":15}` — DIFFERENT key set and order from the non-stream path (§3.3). Both shapes are MUST and pinned by goldens.
...
Recordings: CLIProxyAPI v7.3.4 (docker image digest `sha256:97825da3009f98acf78b5c172fde650a5fbe7a690950a69ce6d7b535d77d4266`), oracle env per `reports/oracle/BOOTSTRAP.md` §3 with the mock fleet of §8. Recording stack (oracle-worker-5): reference on port **8417**, mock fleet on **22999** (openai) and **23001-23007**; all port numbers (and the `host.docker.internal:<port>` base-urls) are masked dynamic fields in fixtures — substitution expected, never byte-compared. Config: openai-compatibility provider `mock-openai` with model `mock-gpt-model` alias `mock-model`, api-key `mock-upstream-key`; `api-keys:["oracle-local-key-1"]`, `request-retry:0`, `transient-error-cooldown-seconds:-1`, `usage-statistics-enabled:false`. `oracle-local-key-1` in fixtures is not a secret.
...
Case definitions with exact requests and scripted upstream replies: `spec/recordings/S2d6.cases.json`. Fixture counts are updated by @oracle-runner recording results.

DEFERRED (specified, not recorded):
- S2d6-stream-slow — identical bytes to S2d6-stream-basic; only inter-event timing differs (mock `slow` mode, 300 ms). OPTIONAL follow-up recording; if recorded, timing goes to `meta.yaml` dynamic fields, never to byte assertions.
...
2. **Stream 429 sanitization** (§5.1): whether the pre-first-frame stream error body is sanitized-and-resorted vs verbatim is source-derived; golden S2d6-error-stream-429 is authoritative. Open question for the adversary gate: confirm the recorded bytes match `{"error":{code,message,type}}` key order.
...
5. `EnsureResponsesUsageDetails` is NOT applied on the stream path (§3.5 asymmetry: stream usage lacks `output_tokens_details` when the upstream never sent reasoning tokens). MUST reproduce; S7 candidate.
