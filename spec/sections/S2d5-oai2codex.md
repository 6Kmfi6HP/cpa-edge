...
- Extra fixed headers from the `codex-api-key` entry `headers` map are applied after the whitelist and may override it (OPTIONAL config; evidence `util.ApplyCustomHeadersFromAttrs`) — EXCEPT `User-Agent` and `Originator`, which the default cloaking pass (running LAST) overrides regardless. `Chatgpt-Account-Id` is set ONLY for OAuth credentials, never for `codex-api-key` (FIXTURE-DEFERRED; evidence `applyCodexHeadersFromSources`).
...
- `response.done` is a terminal ALIAS accepted by the stream executor's terminal switch (evidence: `codex_executor_stream.go` `case "response.completed", "response.incomplete", "response.done"`), but it is a websocket-transport marker — out of scope for HTTP goldens. It is RENAMED before translation (`normalizeCodexWebsocketCompletion` rewrites `type` to `response.completed`, `codex_websockets_errors.go`), so a `response.done`-terminated stream emits a NORMAL terminal finish chunk; the non-stream aggregation does NOT treat it as terminal (`codex_executor_execute.go` switch omits it).
...
- For every tool of type `function` or `custom` that carries an object `parameters` (namespace tools recurse into their nested `tools`):
  1. **Pattern strip** (evidence `stripIncompatiblePatternsFromJSON`/`stripIncompatiblePatterns`): the whole `parameters` object is parsed ONLY when its raw text contains `\p{`, `\P{` or a plain `\u` escape (the `\u` substring alone triggers the parse attempt). Inside, `pattern` values containing Unicode property escapes are deleted, and `patternProperties` KEYS whose regex contains such an escape are deleted too (their sub-schemas recurse); traversal visits only schema-keyword locations (`SchemaMapKeywords`/`SchemaValueKeywords`), never user data like `description`/`default`/`enum` values. When ANY deletion fires, the whole `parameters` object is RE-SERIALIZED (Go map encoding, HTML escaping off) — keys come out ALPHABETICALLY ordered, which is part of the observable wire shape.
...
| E6 | `response.incomplete` terminal with zero output (no output items, no meaningful deltas, `usage.output_tokens` == numeric 0) | Gateway error (pre-commit 502 / in-stream after payload): message `stream error: upstream terminated with incomplete empty response (0 tokens)`. Evidence: `helps/codex_terminal_incomplete.go`. Golden S2D5-26 (pre-commit 502, byte-recorded). |
...
1. **Session UUID derivation** — the derivation chain (caller-scope SHA-256 → `ctx:v1:` SHA-256 canonical root → SHA-1 UUID v5 over `"cli-proxy-api\x00codex\x00derived-session\x00<root>"` under the OID namespace) is complex and version-sensitive. CPA-Edge MUST satisfy the observable contract (§2.9) but MAY pick a different derivation algorithm; exact-UUID mirroring is OPTIONAL. Contract tests mask the value.
