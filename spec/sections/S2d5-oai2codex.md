...
- array content per part type: `text`→`input_text`/`output_text`; `image_url`→`{"type":"input_image","image_url":…,["detail":…],"file_id"…}` (USER role only); `file`→`{"type":"input_file","file_data":…,["filename":…]}` (user only; requires `file.file_data`); `input_audio`→`{"type":"input_audio","data":…,["format":…]}` (user only, requires data).
...
| E2 | Upstream 429 additionally arms the rate-limit cooldown (reset ≈1s) — NOT disabled by `transient-error-cooldown-seconds: -1`; the NEXT request within the window on the same model gets HTTP 500 `{"error":{"code":"model_cooldown","last_upstream_error":"<verbatim upstream body>","message":"All credentials for model <model> are cooling down via provider codex (last error: …)","model":…,"reset_time":…}}` | [RECORDED shape (gemini proof; same machinery)] S4 owns the algorithm; golden S2D5-24 (pair recording). |
