...
- **Non-equivalence NE-LENIENT (2026-09-16, from S2d2 round-1 evidence):** upstream parses request bodies leniently (gjson best-effort — a truncated-JSON body was recorded as 200 with full translation). CPA-Edge enforces a STRICT request boundary: non-JSON / malformed bodies are rejected with 400 (per each surface's error shape). Contract fixtures replay well-formed bodies only.
- **S7 admitted (2026-09-16, gate round 3):** the authoritative platform-degradation registry is spec/sections/S7-platform-degradation.md §7 (NE-S7-01..11) + its matrix (F1-F8 with per-runtime classifications). BINDING consequences for downstream steps: RuntimeCapabilities descriptor is core-owned architecture (I-core implements; runtimes declare constants); device flows are cloudflare EQUIVALENT-conditional (Store-backed sessions + DO alarms) and vercel DEGRADED (NE-S7-11); usage RESP protocol is the node-runtime contract (F8); TLS is platform-terminated on serverless (F7); serverless management REQUIRES allow-remote: true (D1 states it); GET /config mirrors stored values incl. secrets (D1 security warning).
- **D2 GAP RULINGS (2026-09-16, final audit disposition — v1 scope re-scoped honestly per the (b) recommendation):**
- **GR-1 OAuth serving (M1, REGISTERED):** v1 serves API-KEY upstreams (gemini/claude/codex/openai-compat). OAuth credential types (Claude/Codex/Antigravity/Kimi/xAI/Meta/Devin) load, list, patch, refresh, and expire per S3/S6 — but no client-facing model maps to them and no facade accepts them; a request for an OAuth-only model answers 400 model_not_found. CLI-subscription serving is the primary v1.1 item.
- **GR-2 Antigravity executor (M2, REGISTERED, supersedes R-EXECS's antigravity clause):** S2d10's login/redirect/callback surface is PINNED (S3/S7); its executor REQUEST wire is recorded (18 goldens, R-SYNCREDS) but UNIMPLEMENTED in v1. The I-tr-S2d10 step remains pending with a complete golden spec.
- **GR-3 Native passthrough seams (M10, REGISTERED):** messages:claude-api-key (Claude->Claude) and v1beta:gemini-api-key (Gemini->Gemini) — plus xai/meta/interactions/vertex native wires — answer 503 {"error":{"message":"direction not yet available in this build","type":"server_error","code":"not_implemented"}} (the seam body is the registered v1 shape; R-EXECS's "S7-style 501" wording is superseded by this entry).
- **GR-4 Scheduling runtime (M4, REGISTERED):** the S4 strategy engine (round-robin/weighted/fill-first/session-affinity/tiers) is merged in @cpa-edge/core with recorded-chain unit pins, but the v1 serving path performs config-order first-fit with per-facade cooldown skip + request-retry rotation; strategy/weights/affinity switches are management-CRUD echo-only at request time. Multi-account strategy wiring is v1.1.
- **GR-5 Node runtime scope (M6/M7/M8, PARTIALLY FIXED):** hot-reload recompose + proxy-url ingestion (fail-closed strip + warning; actual proxy dialing REGISTERED absent — node fetch cannot proxy without undici agent plumbing) + the /v1/ws recorded gates land in the T1 parity round. Remaining REGISTERED absent on node: background token-refresh driver (manual POST /auth-files/refresh is the v1 path), device-flow poll driver (node device sessions = envelope-only, vercel NE-S7-11-equivalent), file-log substrate (Store ring serves /logs), TLS listener (library hosts own TLS), redirect-flow loopback forwarders (manual relay via the pinned oauth-callback endpoint).
- **GR-6 Accepted-inert config (M11, REGISTERED per the S4 quota-exceeded precedent):** payload rules engine, force-model-prefix, per-entry $-dynamic headers outside codex-passthrough, streaming keepalives, quota refresh of request-error-logs — accepted + echoed + round-tripped, no runtime consumer.
- **GR-7 Legacy /v1/completions prompt adaptation (M13, REGISTERED):** unpinned seam; legacy prompt bodies are not converted.
- **GR-8 Unreplayed golden classes (M12, REGISTERED):** S4 (recorded bytes inlined in core unit tests), S7-01..04 (node /v1/ws gates land in the T1 parity round with replay), S2d10 (GR-2).
- **Ruling R-ORDER (2026-09-16, from S2d2 round-2 evidence; restored after registry incident):** upstream flushes multi-tool-call streams via Go map iteration — the ORDER of tool-call parts within a single flush frame is NONDETERMINISTIC. Contract tests comparing frames with 2+ tool-call parts compare the parts as a SORTED SET; single-part frames stay byte-exact ordered. Escalation-session model: cross-case cooldown-history goldens replay as one shared service+Store under a stepwise clock (the s2d2/s2d5 family precedent).
- **Ruling R-SYNCREDS (2026-09-16, from S2d10 review; restored):** OAuth-provider executors that honor credential base_url overrides (e.g. antigravity) are RECORDABLE-LOCALLY for their REQUEST-side wire via synthetic uploaded credentials pointing at local mocks — R-FIXTURE refined. Vendor-issued tokens/consent and vendor response CONTENT remain CREDENTIALED-ONLY.
- **Ruling R-EXECS (2026-09-16; restored):** executor wire behavior for every RECORDED direction is embedded + golden-pinned inside its direction module (gemini x3, claude, openai-chat x3, codex-responses x2, antigravity via S2d10 synthetic credentials). The charter's I-exec steps for openai/claude/gemini/codex/antigravity/custom-openai are SATISFIED by the direction modules. Providers whose NATIVE wires were never recorded (kimi-native, xai-native, meta-native OAuth, interactions, vertex standalone) are a REGISTERED NON-EQUIVALENCE for v1: their credential types load, schedule, and refresh per S3/S4/S6, but native-wire calls return the S7-style 501 degradation (D2 quantifies; R-SYNCREDS recording may enable their specs later).
- **Ruling R-TRACE (2026-09-16, family ruling from the s2d6 contract gate):** X-Cpa-Trace-Id is S1/TRANSPORT-LAYER territory — translator facades NEVER emit it; contract suites NEVER compare it (presence-only recognition). The runtime route layer owns trace emission on executor-routed responses.
